...
; ---- GOLD MAGNET ASSIST ----
; magnet_active/magnet_timer are managed by GOLD TIMER TICK.
; If magnet isn't active, we're done.
lda magnet_active
beq @magnet_done
...
; abs(dx) between player and object
lda player_x
sec
sbc obj_x
bcs @dx_pos
...
; pull 2px toward object (both directions)
lda player_x
cmp obj_x
beq @magnet_done
bcc @mag_right
...
; =========================
; MOVE OBJECT #2 (simple: same fall_speed as obj1)
; =========================
; MOVE OBJECT #2 (use tmp, same as obj1)
lda obj2_alive
beq @obj2_move_done
  lda obj2_y
  clc
...
; if obj2 fell past bottom, deactivate it (and penalize missed GOOD)
lda obj2_alive
beq @obj2_bottom_done

  lda obj2_y
  cmp #OBJ_RESET_Y        ; use same bottom threshold as obj1
  bcc @obj2_bottom_done

  ; If missed GOOD (obj2_type==0), apply penalty
  lda obj2_type
  cmp #OBJ_GOOD
  bne @obj2_just_kill

  ; ---- Missed GOOD => penalty (same as obj1 miss) ----
  lda #$01
  sta combo_mult
  lda #$00
  sta combo_count
  ...
  inc misses
  lda #$00
  sta good_count          ; optional: keep if you want misses to reset this
  ...
  lda misses
  cmp #$03
  bcc @obj2_just_kill     ; not game over yet
...
@done:
  ...
  ; spawn_cd = OBJ2_SPAWN_DELAY + random(0..31)
SetObj2SpawnCD:
  jsr NextRNG
  lda rng
  and #$1F              ; 0..31
