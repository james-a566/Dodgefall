...
; Base fall speeds by difficulty (integer part)
FallSpeedTable:
  .byte 1, 1, 2, 2, 3

; Fractional toggle mask by difficulty:
; 0 = no fractional (always integer)
; 1 = add +1 every other frame (i.e., +0.5 feel)
FracMaskTable:
  .byte 0, 1, 0, 1, 0

; Spawn odds tables using rng & $07 (0..7)
; power happens if == PowerHit[diff]
; else good if < GoodCut[diff]
; else bad
PowerHitTable:
  .byte 1, 1, 1, 7, 7     ; power common early, rarer later
GoodCutTable:
  .byte 5, 4, 4, 3, 3     ; threshold for "good" (0..7)
...
SpawnObject:
  ...
  ; power if roll == PowerHitTable[diff]
  tya
  cmp PowerHitTable,x
  beq @make_power

  ; good if roll < GoodCutTable[diff]
  tya
  cmp GoodCutTable,x
  bcc @make_good
...
@have_thr:
  ...
  ; tmp2 = tmp + 2  (slightly looser window for arming hold)
  lda tmp
  clc
  adc #$02
  sta tmp2

  ; decay hold each frame
  lda catch_hold
  beq :+
  dec catch_hold
:
  ...
  ; If dy >= tmp2, it's not even close -> clear hold and apply
  cmp tmp2
  bcs @cc_clear_and_apply
...
@dx_ok:
  ...
  ; If dx >= tmp2, not even close -> clear hold and apply
  cmp tmp2
  bcs @cc_clear_and_apply

  ; At this point we're "near enough" -> arm hold for 2 frames
  lda #$02
  sta catch_hold

  ; If BOTH dy < tmp and dx < tmp -> catch immediately
  lda absdy
  cmp tmp
  bcs @cc_maybe_hold     ; dy not tight enough
  lda absdx
  cmp tmp
  bcs @cc_maybe_hold     ; dx not tight enough

  jmp @cc_caught_dispatch

@cc_maybe_hold:
  ; If we were close recently, allow catch if still within tmp2 bounds
  lda catch_hold
  beq @cc_apply
  jmp @cc_caught_dispatch
...
@caught_good:
  ...
  ; assume normal feedback
  lda #$00
  sta tmp            ; tmp = 0 (no streak)

; ---- difficulty ramp progress (every 8 good catches) ----
inc streak8
lda streak8
cmp #RAMP_GOOD_TARGET
bcc :+
  lda #$00
  sta streak8
