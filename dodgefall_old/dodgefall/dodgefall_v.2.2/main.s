...
; - four falling object + 3 misses (lives)
...
STREAK_TARGET = $05      ; X goods in a row (5)
SCORE_STREAK  = $50      ; +50 points (BCD)

COMBO_STEP = 3     ; every 3 consecutive GOOD catches, multiplier increases
...
; =========================
SCORE_Y    = $08
HISCORE_Y  = $08    ; or $04 / $10 depending on taste
MULTI_Y    = $18

HUD_SAFE_Y = $28     ; 40px down (pick $20 or $28)


HISCORE_X0 = $70      ; centered

; Multiplier badge uses sprites 62..63 (OAM offsets $F8 and $FC)
MULTI_BASE = $F8        ; sprite 62 * 4
MULTI_X_X  = $28        ; X for 'x' (tweak)
MULTI_X_D  = $30        ; X for digit (tweak)
TILE_X     = $25        ; your CHR tile for the small 'x'
...
SCORE_GOOD   = $10   ; +10
SCORE_POWER  = $25   ; +25
SCORE_GOLD   = $50   ; +50  (tweak)
SCORE_BAD    = $10   ; -10

; tiles (we’ll add 2 new tiles in CHR)
TILE_SOLID   = $00   ; your existing solid block (color index 3)
TILE_POWER   = $23   ; NEW: solid block using color index 1
TILE_GOLD    = $24   ; NEW: solid block using color index 2

TRAIL_BASE = $D4        ; sprite 53
TRAIL_ATTR = $02        ; pick a palette that looks dimmer (0..3)
TRAIL_DY   = $02        ; how far “behind” (2 px)
TRAIL_MIN_SPEED = $03   ; show trail when tmp >= 3
...
player_gold_timer: .res 1   ; nonzero => use TILE_GOLD for player
...
streak_chirp: .res 1   ; 0 = off, 1 = waiting for 2nd note

diff:          .res 1   ; 0..4 difficulty level
ramp_lock:     .res 1   ; grace frames after damage (prevents ramping)
streak8:       .res 1   ; counts good catches toward ramp (0..7)
fall_frac:     .res 1   ; toggles for fractional speed (0/1)

catch_hold: .res 1   ; frames remaining (0..2)
absdx:      .res 1
absdy:      .res 1
tmp2:       .res 1   ; secondary threshold (tmp + 2)

combo_count: .res 1
combo_mult:  .res 1     ; 1..4 (or whatever cap you want)
...
magnet_timer: .res 1   ; frames of magnet assist remaining

gold_boost_timer: .res 1    ; >0 => player speed boost active

obj_y_prev: .res 1

magnet_active: .res 1   ; 0/1 gate for magnet
...
MaybeIncreaseDifficulty:
  ...
  ; optional feedback: a little beep when difficulty increases
  jsr PlaySpeedupBeep
...
MAX_DIFF = 4
RAMP_LOCK_FRAMES = 90   ; ~1.5 seconds at 60fps
RAMP_GOOD_TARGET = 8
...
@do_game:
  ...
  lda #$02
  sta tmp               ; tmp = player speed (2)

  lda gold_boost_timer
  beq :+
    lda #$03
    sta tmp             ; boosted speed (3)
:
...
; ---- magnet timer tick (always runs during gameplay) ----
lda magnet_timer
beq @mag_tick_done
dec magnet_timer
bne @mag_tick_done
...
; ---- GOLD MAGNET ASSIST (tuned) ----
lda magnet_active
beq @magnet_done

  ; don't pull toward bad blocks
  lda obj_type
  cmp #OBJ_BAD
  beq @magnet_done

  ; engage earlier in the fall
  lda obj_y
  cmp #$80       
  bcc @magnet_done
  cmp #$E0
  ...
  ; only pull if we're >= 4px away (avoid "invisible" micro pulls)
  lda player_x
  sec
  sbc obj_x
  bcs @dx_pos
...
jsr AddScoreWithMultA
  ...
  lda combo_mult
  cmp #COMBO_MAX
  bcs @combo_feedback_only     ; already at max, don't increase

  inc combo_mult               ; safe increase
  ...
  ; if we just hit max, start badge flash
  lda combo_mult
  cmp #COMBO_MAX
...
ApplyTimerCheck:
; ---- request ramp every ~12 seconds (720 frames = $02D0) ----
lda frame_hi
cmp #$03
